...
class PickPlace(ManipulationEnv):
    ...
    def _get_env_info(self, action):
        ...
        # for i in range(len(self.objects_in_bins)):
        #     info['obj{}_in_bin'.format(i)] = float(self.objects_in_bins[i])
        info["num_objs_in_bins"] = np.sum(self.objects_in_bins)

        r_reach, r_grasp, r_lift, r_hover = self.staged_rewards()
        ### normalize rewards for logging ###
        info["r_reach"] = r_reach / 0.1
        info["r_grasp"] = r_grasp / 0.35
        info["r_lift"] = r_lift / 0.5
        info["r_hover"] = r_hover / 0.7
    ...
    def _get_skill_info(self):
        ...
        for i, obj in enumerate(self.objects):
            if self.single_object_mode in {1, 2} and i != self.object_id:
                continue

            obj_str = obj.name
            obj_pos = np.array(self.sim.data.body_xpos[self.obj_body_id[obj_str]])

            bin_pos = self.target_bin_placements[0].copy()
            bin_pos[0] += self.bin_size[0] / 4
            bin_pos[1] += self.bin_size[1] / 4

            drop_pos = bin_pos + [0, 0, 0.20]
            pos_info["reach"].append(drop_pos)
            pos_info["grasp"].append(obj_pos)
    ...
    def _get_observation(self):
        ...
        if self.use_object_obs:
            ...
            for i, obj in enumerate(self.objects):
                ...
                obj_str = obj.name
                obj_pos = np.array(self.sim.data.body_xpos[self.obj_body_id[obj_str]])
                obj_quat = T.convert_quat(self.sim.data.body_xquat[self.obj_body_id[obj_str]], to="xyzw")
