...
class Door(ManipulationEnv):
    ...
    def _get_env_info(self, action):
        ...
        success = hinge_qpos > 0.3
        d_reach = np.linalg.norm(self._gripper_to_handle)
        r_reach = 1 - np.tanh(10.0 * d_reach)
        info.update(
            {
                "hinge_qpos": hinge_qpos,
                "success": success,
                "d_reach": d_reach,
                "r_reach": r_reach,
            }
        ...
        if self.use_latch:
            handle_qpos = self.sim.data.qpos[self.handle_qpos_addr]
            r_handle = np.clip(np.abs(handle_qpos / (0.5 * np.pi)), -1, 1)
            info.update(
                {
                    "handle_qpos": handle_qpos,
                    "r_handle": r_handle,
                }
            )
        return info
    ...
    def _get_skill_info(self):
        ...
        pos_info["grasp"] = [handle_pos]  # grasp target positions
