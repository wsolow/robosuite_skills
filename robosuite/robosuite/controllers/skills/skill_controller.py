...
import copy
import numpy as np
from robosuite.controllers.skills.skills import (
    AtomicSkill,
    ReachSkill,
    ReachOSCSkill,
    GraspSkill,
...
class SkillController:
    ...
    def _setup_config(self, config):
        ...
        skill_names = [skill_name for skill_name in SkillController.SKILL_NAMES if skill_name in skill_names]
        ...
        for skill_name in skill_names:
            ...
            if skill_name == "atomic":
                ...
                raise ValueError
            skill_config.update(self._config.get(skill_name + "_config", {}))
    ...
    def reset(self, action):
        ...
        robot_control_dim = np.sum(
            [
                robot.composite_controller.get_control_dim(p)
                for p in robot.composite_controller.part_controller_config.keys()
            ]
        )
        ...
        skill_config_update = dict(
            ...
            robot_gripper_dim=gripper_control_dim,
            robot_controller=robot.composite_controller,
        )
        info = self._get_info()
    ...
    def _get_info(self):
        ...
        info["cur_ee_pos"] = np.array(
            [robot.sim.data.site_xpos[robot.eef_site_id[k]] for k in robot.eef_site_id.keys()]
        )
        return info
