...
class SACHybridTrainer(SACTrainer):
    def __init__(self, target_entropy_s=None, target_entropy_p=None, target_entropy_config=None, **sac_kwargs):
    ...
    def compute_loss(
        ...
        if self.use_automatic_entropy_tuning:
            if log_pi_s is not None:
                alpha_s_loss = -self.log_alpha_s * self.reduce_tensor(log_pi_s + self.target_entropy_s, dd).detach()
                alpha_s = self.log_alpha_s.exp()
            else:
                alpha_s_loss = None
                alpha_s = None

            alpha_p_loss = -self.log_alpha_p * self.reduce_tensor(log_pi_p + self.target_entropy_p, dd).detach()
