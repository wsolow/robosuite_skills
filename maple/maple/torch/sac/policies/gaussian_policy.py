...
from maple.torch.networks.stochastic.distribution_generator import DistributionGenerator
...
class TanhGaussianPolicy(Mlp, TorchStochasticPolicy):
    ...
    def __init__(self, hidden_sizes, obs_dim, action_dim, prefix="", std=None, init_w=1e-3, **kwargs):
    ...
    def forward(self, obs):
        ...
        return TanhNormal(mean, std, prefix=self.prefix)

    # def logprob(self, action, mean, std):
    #     tanh_normal = TanhNormal(mean, std, prefix=self.prefix)
...
class GaussianCNNPolicy(CNN, TorchStochasticPolicy):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            hidden_sizes=hidden_sizes, output_size=action_dim, init_w=init_w, output_activation=torch.tanh, **kwargs
        )
        self.min_log_std = min_log_std
        self.max_log_std = max_log_std
        self.log_std = None
