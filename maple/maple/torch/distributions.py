...
class GaussianMixture(Distribution):
    def __init__(self, normal_means, normal_stds, weights):
        ...
        self.normals = [
            MultivariateDiagonalNormal(normal_means[:, :, i], normal_stds[:, :, i]) for i in range(self.num_gaussians)
        ]
...
class TanhNormal(Distribution):
    ...
    def _log_prob_from_pre_tanh(self, pre_tanh_value):
        ...
        correction = -2.0 * (
            ptu.from_numpy(np.log([2.0])) - pre_tanh_value - torch.nn.functional.softplus(-2.0 * pre_tanh_value)
        ).sum(dim=-1)
    ...
    def rsample_with_pretanh(self):
        z = (
            self.normal_mean
            + self.normal_std
            * MultivariateDiagonalNormal(ptu.zeros(self.normal_mean.size()), ptu.ones(self.normal_std.size())).sample()
        )
        return torch.tanh(z), z
    ...
    def get_actions_and_logprobs(self):
        ...
        expansion_factor = 1
        return (expansion_factor, value.reshape((bs, -1, action_dim)), log_p.reshape((bs, -1)))
