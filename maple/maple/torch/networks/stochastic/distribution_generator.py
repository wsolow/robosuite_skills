...
class ModuleToDistributionGenerator(MultiInputSequential, DistributionGenerator, metaclass=abc.ABCMeta):
    pass
...
class Gaussian(ModuleToDistributionGenerator):
    ...
    def forward(self, *input):
        if self.std:
            ...
            std = log_std.exp()
        return MultivariateDiagonalNormal(mean, std, reinterpreted_batch_ndims=self.reinterpreted_batch_ndims)
