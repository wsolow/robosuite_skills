...
from maple.policies.base import Policy
from maple.pythonplusplus import identity
from maple.torch import pytorch_util as ptu
from maple.torch.core import PyTorchModule, eval_np
from maple.torch.data_management.normalizer import TorchFixedNormalizer
from maple.torch.networks import LayerNorm
from maple.torch.pytorch_util import activation_from_string
...
class MlpGoalQfWithObsProcessor(Mlp):
    def __init__(self, obs_processor, obs_dim, backprop_into_obs_preprocessor=True, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.obs_processor = obs_processor
...
class SplitIntoManyHeads(nn.Module):
    ...
    def __init__(
        ...
        if output_activations is None:
            ...
            if len(output_activations) != len(output_sizes):
                raise ValueError("output_activation and output_sizes must have " "the same length")
    ...
    def forward(self, flat_outputs):
        pre_activation_outputs = tuple(
            flat_outputs.narrow(1, start, length) for start, length in self._output_narrow_params
        )
        outputs = tuple(activation(x) for activation, x in zip(self._output_activations, pre_activation_outputs))
        return outputs
