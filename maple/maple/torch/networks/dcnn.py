...
class TwoHeadDCNN(PyTorchModule):
    def __init__(
        ...
        for out_channels, kernel_size, stride, padding in zip(n_channels, kernel_sizes, strides, paddings):
            deconv = nn.ConvTranspose2d(
                deconv_input_channels, out_channels, kernel_size, stride=stride, padding=padding
            )
    ...
    def forward(self, input):
        h = self.apply_forward(
            input, self.fc_layers, self.fc_norm_layers, normalization_type=self.fc_normalization_type
        )
        ...
        h = self.apply_forward(
            h, self.deconv_layers, self.deconv_norm_layers, normalization_type=self.deconv_normalization_type
        )
    ...
    def apply_forward(self, input, hidden_layers, norm_layers, normalization_type="none"):
        h = input
...
class BasicDCNN(PyTorchModule):
    ...
    def __init__(
        ...
        for i, (out_channels, kernel_size, stride, padding) in enumerate(
            ...
            deconv = nn.ConvTranspose2d(input_channels, out_channels, kernel_size, stride=stride, padding=padding)
            if hidden_init:
                hidden_init(deconv.weight)
    ...
    def forward(self, input):
        h = input.view(-1, self.input_channels, self.input_width, self.input_height)
