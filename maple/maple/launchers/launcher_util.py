...
from maple.launchers import conf
from maple.torch.pytorch_util import set_gpu_mode
...
def run_experiment(
    ...
    :param use_gpu:
    :param snapshot_mode: See maple.core.logging
    :param snapshot_gap: See maple.core.logging
    ...
    if mode == "local":
        ...
        credentials = doodad.ssh.credentials.SSHCredentials(
            username=ssh_dict["username"], hostname=ssh_dict["hostname"], identity_file=conf.SSH_PRIVATE_KEY
        )
        ...
        dmode = doodad.mode.EC2AutoconfigDocker(
            ...
            # will start just making the sub-directories within maple rather
        ...
        if exp_folder is not None:
            gcp_log_path = osp.join(gcp_log_path, exp_folder)
