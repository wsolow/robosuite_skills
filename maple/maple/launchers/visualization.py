...
def dump_video(
    ...
    filename = osp.join(logdir, "video_{mode}_{epoch}.mp4".format(mode=mode, epoch=epoch))
    import imageio
    ...
    dump_skillmap(env, rollout_actions, successes, horizon, logdir, mode, epoch)
...
def dump_skillmap(
    ...
    filename = osp.join(logdir, "cmap_{mode}_{epoch}.png".format(mode=mode, epoch=epoch))
...
def add_border(img, pad_length, pad_color, imsize=84):
    ...
    img2 = np.ones((H + 2 * pad_length, W + 2 * pad_length, img.shape[2]), dtype=np.uint8) * np.array(
