...
from maple.envs.wrappers import ProxyEnv
...
class VAEWrappedEnv(ProxyEnv, MultitaskEnv):
    ...
    def _update_info(self, info, obs):
        latent_distribution_params = self.vae.encode(ptu.from_numpy(obs[self.vae_input_observation_key].reshape(1, -1)))
        latent_obs, logvar = (
            ptu.get_numpy(latent_distribution_params[0])[0],
    ...
    def sample_goals(self, batch_size):
        ...
        if self._goal_sampling_mode == "custom_goal_sampler":
            ...
            sampled_goals = {k: v[idx] for k, v in self._presampled_goals.items()}
        ...
        if self._decode_goals:
            ...
            decoded_goals = None
        image_goals, proprio_goals = self._image_and_proprio_from_decoded(decoded_goals)
    ...
    def goal_sampling_mode(self, mode):
        assert mode in ["custom_goal_sampler", "presampled", "vae_prior", "env", "reset_of_env"], "Invalid env mode"
        ...
        if mode == "custom_goal_sampler":
            ...
            if test_goals is None:
                ...
                warnings.warn("self.goal_sampler returned None. " + "Defaulting to vae_prior goal sampling mode")
    ...
    def custom_goal_sampler(self, new_custom_goal_sampler):
        assert self.custom_goal_sampler is None, "Cannot override custom goal setter"
    ...
    def try_render(self, obs):
        if self.render_rollouts:
            ...
            cv2.waitKey(1)
            init_reconstruction = self._reconstruct_img(self._initial_obs["image_observation"]).transpose()
