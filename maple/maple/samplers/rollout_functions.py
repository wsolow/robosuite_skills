...
def contextual_rollout(env, agent, observation_key=None, context_keys_for_policy=None, obs_processor=None, **kwargs):
...
def rollout(
    ...
    while path_length < max_path_length:
        ...
        next_o, r, d, tr, env_info = env.step(copy.deepcopy(a), image_obs_in_info=image_obs_in_info)
    ...
    path_length_actions = np.sum([info.get("num_ac_calls", 1) for info in env_infos])

    reward_actions_sum = np.sum([info.get("reward_actions", 0) for info in env_infos])
...
def deprecated_rollout(
    ...
    while path_length < max_path_length:
        ...
        next_o, r, d, tr, env_info = env.step(a)
        observations.append(o)
        rewards.append(r)
        terminals.append(d or tr)
        actions.append(a)
